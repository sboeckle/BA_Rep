	...
	public static void main(String args[]) {
		if (args.length != 2)
			throw (new RuntimeException(
					"Usage : java FileCompare fileName1 fileName2"));
		...
		try {
			if (CompareFilesbyByte(fileName1, fileName2) == true)
				System.out
						.println("No Difference encountered  using CompareFilesbyByte method");
			else
				System.out
						.println("Both Files are not equal using CompareFilesbyByte method");
		} catch (IOException e) {
		...
		try {
			...
			if (hashValue.equals(hashValue1))
				System.out
						.println("No Difference encountered using method MD5Hash");
			else
